...
3. **Initialize logging**: resolve CMTrace log path (`HKLM:\SOFTWARE\Microsoft\CCM\Logging\@Global\LogDirectory` → `%SystemRoot%\CCM\Logs\ClearCache.log` → `%TEMP%\ClearCache.log`), rotate to `.lo_` if > 10 MB. Snapshot cache utilization (disk size from a recursive walk; configured max from `CacheConfig.Size`, in MB) and emit a verbose+CMTrace line — read by admins before deciding to dig into the per-item output.
